...
- **Environment Variables** (passed from main.py):
  ...
  # Random ports (generated internally on remote VPS)
  # XUI_WEB_PORT, XUI_SUB_PORT, CADDY_GLOBAL_INTERNAL_PORT,
  # TCP_REALITY_INBOUND_PORT, XHTTP_REALITY_INBOUND_PORT are generated by generate_ports()
...
| `/{SECRET_SUB_PATH}/api/client` | POST | Add/edit/delete client | Yes | Actions: `add`, `delete`, `edit` (renames or moves client across nodes in `nodes.json` and updates `force-subs.yml`) |
| `/{SECRET_SUB_PATH}/api/node` | POST | Add/edit/delete node | Yes | Actions: `add`, `delete`, `edit` (renames node or updates backend URL in `nodes.json`) |
| `/{SECRET_SUB_PATH}/api/reset` | POST | Reset sync statuses | Yes | Resets all client synchronization tracking states to pending |
...
- **Web UI Dashboard**:
  ...
  - Client synchronization status indicator: badges and live tracking (`synced`, `pending`, `stale`, `unassigned`)
  - Interactive filter buttons in UI (All, Synced, Stale, Unassigned) and sync reset tool
  - In-place modal editing for client names/nodes and node names/URLs
  ...
  - Dark theme, refined responsive styling with custom scrollbars
...
3. **Saved VPS Profiles Manager (PIN-encrypted `servers.json`)**: 
   - The UI includes a slide-out "Servers" drawer to save, edit, lock, and reload server credentials for any host role (`proxy_host`, `freedom_host`, `sub_vps_host`, `vps_host`, `backup_vps_host`, etc.).
   - Security & Protection: Uses client-side PBKDF2 (100,000 iterations) + AES-GCM encryption with a user-defined PIN. Individual server cards support a lock toggle (`locked: true`) preventing accidental deletion or overwriting.
...
6. **Asynchronous Version, Backup & Update Check**:
   ...
   - `GET /api/update_check`: Checks for new repository releases on GitHub and displays an update notification banner in the UI header.
...
8. **Dynamic Visual Topology Rendering & Animated SVG Rocket**: 
   ...
   - Dedicated vector illustration assets in `resources/rocket/` (`rocket.css`, `rocket.js`) provide smoothly animated rocket elements for deployment stages, headers, and topology nodes without static emoji placeholders.
...
3. **Fragile Remote Output Parsing in Deployer** — *RESOLVED*:
   - Remote `panel/setup.sh` now emits a machine-readable JSON summary block enclosed in `===RESULT_JSON_START===` and `===RESULT_JSON_END===`.
   - `ssh_deployer.py:parse_deployment_results()` parses this structured JSON directly for panel credentials and client keys/subscriptions (with robust legacy fallback), eliminating reliance on localized string parsing (`"Клиент:"`, `"Client:"`).

4. **Template Substitution Edge Cases (`sed` vs Structured Configs)** — *RESOLVED*:
   - Remote scripts `panel/setup.sh` and `sub-server/setup.sh` now include variable sanitization (`escape_sed_replacement()` / safe value substitution) preventing special characters (`|`, `&`, `\`) in passwords, secrets, or paths from breaking `sed` template replacements.
   - Inbound configurations use robust JSON-encoded payloads for short IDs and client lists.

5. **Thread Safety in Local Control Plane (`main.py`)** — *RESOLVED*:
   - Added `DEPLOY_LOCK = threading.Lock()`, `LOG_CONDITION = threading.Condition(DEPLOY_LOCK)`, and `CACHE_LOCK = threading.Lock()` to synchronize state across `ThreadingHTTPServer` worker threads and background deployment tasks.
   - Deployment lifecycle (`is_deploying`, `deploy_status`, `deploy_result`, `cancel_requested`) transitions and check-then-act operations in `POST /api/deploy`, `/api/deploy/stop`, `/api/restart`, `/api/shutdown`, and `/api/update_sources` are executed under `DEPLOY_LOCK`.
   - SSE log streaming at `GET /api/deploy/logs` now reactively awaits new events via `LOG_CONDITION.wait(timeout=1.0)` instead of busy-polling `time.sleep(0.5)`, eliminating latency and race conditions on `active_logs`.
   - `fetch_xui_versions()` and `check_for_update()` caches (`XUI_CACHE`, `UPDATE_CACHE`) are protected under `CACHE_LOCK`.
...
10. **Dead Configuration & Dual-Mode UI Rationale** — *RESOLVED*:
    - Dead fields and unused env vars (`EMAIL`, `XUI_WEB_PORT`, `XUI_SUB_PORT` in `ssh_deployer.py`, `sub_same_as_proxy` in `main.py` / `app.js`) were removed.
    - Ports are generated strictly on the remote VPS inside `panel/setup.sh` (`generate_ports()`).
    - `freedom_component` vs `freedom_only` / `single` is **by design**: while executing the same underlying deploy sequence, they represent two distinct business use cases in the UI (standalone single-node deployment vs an intermediate egress node intended to be linked with a Russian proxy node in a cascade).

11. **Backup Unification & TLS Scope Design Rationale** — *RESOLVED*:
    - Unified `_perform_remote_backup` and `_perform_remote_sub_backup` into a single parameterized helper `_perform_remote_backup(..., target="panel" | "sub_server")` in `ssh_deployer.py`, eliminating duplicate remote script execution and SCP download logic.
    - Omission of `.caddy_data` from panel backups is **by design**: panel backups are frequently restored onto new VPS instances or with changed/migrated domains, where Caddy must provision fresh TLS certificates without conflicts or state corruption from old certificates. Sub-server backups retain registry state and TLS configuration.
...
13. **Master Password & PBKDF2 V2 Hardening with Auto-Migration** — *RESOLVED*:
    ...
    - Upgraded PBKDF2 derivation from 100k iterations (`"3x-ui-salt-v1"`) to **600 000 iterations** (`"3x-ui-salt-v2"`, OWASP standard) with AES-GCM 256-bit encryption.
    - Implemented transparent auto-migration: upon entering the existing PIN/password, legacy V1 profiles are automatically detected, decrypted via V1 key, re-encrypted via hardened V2 key, and saved back to `servers.json`.

14. **Cryptographically Secure Session Secret** — *RESOLVED*:
    - In sub-server (`server.py`), `AUTH_SESSION_SECRET` now defaults to `secrets.token_hex(32)` (256-bit cryptographically secure random key) when not explicitly set via environment variables.
    - Password authentication remains completely unaffected (`ADMIN_PASSWORD`), while HMAC-SHA256 session cookies cannot be forged or brute-forced offline.

15. **Inoperative Host Port Availability Check in `panel/setup.sh`** — *RESOLVED*:
    - Fixed subshell return code evaluation in `get_unique_random_port()` by using direct command execution `if [[ "$already_chosen" -eq 1 ]] || is_port_busy "$port"; then`.

16. **Race Conditions on Node Registry and Overrides in `sub-server/server.py`** — *RESOLVED*:
    - Introduced `DATA_LOCK = threading.Lock()` wrapping all mutations and concurrent reads of `NODES` and `FORCE_SUBS` across `ThreadingHTTPServer` worker threads.

17. **Trailing Slash Concatenation in Sub-Server Raw Subscription List** — *RESOLVED*:
    - In `sub-server/server.py: _list_subscriptions()`, node URLs are normalized with `.rstrip('/')` preventing double slashes.

18. **Directory Prefix Traversal Vulnerability in `main.py` Static File Handler** — *RESOLVED*:
    - In `main.py`, path checking now uses `target_file == base_dir or target_file.startswith(base_dir + os.sep)`.

19. **Cross-Population Bug in Backup Selectors (`fetchBackupList` in `app.js`)** — *RESOLVED*:
    - In `panel/static/app.js: fetchBackupList()`, dropdown targets are explicitly filtered by `folder === 'backups_panel'` and `folder === 'backups_sub_server'`, preventing cross-contamination of select elements.

20. **Missing Recovery and Rollback Fields in `save_backup_config()`** — *RESOLVED*:
    - Added `recovery_xui_username` to `recovery_node`, `foreign_sub_url` to `proxy_node` / `panel_and_clients`, and `rollback_sub_backup_file` to `sub_server` in `save_backup_config()` pick lists in `main.py` and `app.js` autosave.

21. **Unsafe Direct Dictionary Access on Node Clients in `sub-server/server.py`** — *RESOLVED*:
    - Replaced all direct `node["clients"]` accesses with `node.get("clients", [])` across all helper functions in `sub-server/server.py`.

22. **False Positive Update Banner via Unexcluded Runtime Files in `main.py`** — *RESOLVED*:
    - Excluded runtime directories (`working`, `backup`) and local state/log files from `_is_code_file` / `EXCLUDED_DIRS` in `main.py`.

23. **Wiping of Pre-downloaded `.python_env` during Source Update** — *RESOLVED*:
    - Added `.python_env` to the preserve list in both `update_sources.sh` and `update_sources.ps1`.

24. **Unsynchronized `CLIENT_ACTIVITY` Dictionary Access in `sub-server/server.py`** — *RESOLVED*:
    - Introduced `ACTIVITY_LOCK = threading.Lock()` guarding `CLIENT_ACTIVITY` read, write, and clear operations.

25. **Open Redirect Weakness in Sub-Server Login** — *RESOLVED*:
    - In `sub-server/server.py: _safe_next_path()`, `next_path` is strictly checked to ensure it starts with `/{SECRET_SUB_PATH}`.

26. **Omission of Empty Environment Variables in Remote Deployers** — *RESOLVED*:
    - In `ssh_deployer.py` (`_deploy_node` and `_deploy_sub_server`), changed `if v:` to `if v is not None:` so empty string variables are preserved.

27. **`cascade_sub` Mode Can Never Pass Form Validation** — *RESOLVED*:
    - Form validation in `panel/static/app.js` was adjusted to correctly handle `cascade_sub` without requiring hidden sub-only inputs.

28. **`get_bundle_bytes()` Ships Runtime State & Backups to Remote** — *RESOLVED*:
    - Extended the exclusion list in `ssh_deployer.py: get_bundle_bytes()` to omit `servers.json`, `backups_panel`, `backups_sub_server`, `nodes.json`, `force-subs.yml`, and `subs.yml`.

29. **`wait_for_lock()` Has No Timeout** — *RESOLVED*:
    - Added a bounded retry limit (timeout loop) in `common/setup.sh: wait_for_lock()` with `die` on timeout.

30. **`common/setup.sh` Unconditionally Overwrites `/etc/docker/daemon.json` and Restarts the Docker Daemon** — *RESOLVED*:
    - `common/setup.sh` checks if `dh-mirror.gitverse.ru` is already configured in `/etc/docker/daemon.json` before overwriting or restarting Docker.

31. **`exec_command()` Cancel Path Leaks Subprocess (Zombie)** — *RESOLVED*:
    - Added `await proc.wait()` after `proc.kill()` in `ssh_deployer.py` cancel and exception handlers.

32. **Stored XSS in Saved-Servers Drawer and Result Cards** — *RESOLVED*:
    - In `panel/static/app.js`, server card rendering, summary result cards, and client links now properly sanitize dynamic fields (`srv.host`, `srv.user`, `url`, `user`, `pass`, `client.name`, `client.group`) using `escapeHtml()`, avoiding double-escaping.

33. **Sub-Server `setup.sh` Echoes `ADMIN_PASSWORD` to Deployment Log** — *RESOLVED*:
    - Removed plaintext password echo from `sub-server/setup.sh: print_results()`.

34. **`update_xray_routing()` May Hit a Dead Port After `webPort` Change** (*NON-ISSUE / AS DESIGNED*):
    - Verified behavior: 3x-UI API listener remains on port 2053 during setup until container restart, so `panel_api_request` on `PANEL_API_PORT` works as expected.

35. **`wait_for_lock()` in `common/setup.sh` Is Inert Due to ERE `\|` Misuse** — *RESOLVED*:
    - Fixed ERE alternation in `pgrep` commands by using plain `|` (`pgrep -f "apt-get|dpkg"`, `pgrep -f "$pm|rpm"`).

36. **`/api/ssh/test` Leaks the asyncio Event Loop on Exception** — *RESOLVED*:
    - Wrapped loop execution in `try ... finally: loop.close()` in `main.py` ensuring the event loop is always closed even on connection exceptions.

37. **`load_past_activity_from_logs()` Truncates Node Names Containing Parentheses** — *RESOLVED*:
    - Updated `sync_re` regex in `sub-server/server.py` to match full node names containing nested parentheses.

38. **`int()` Port Conversion Guards Are Inconsistent in `ssh_deployer.py`** — *RESOLVED*:
    - Standardized all port integer conversions to use `config.get(...) or 22` across `ssh_deployer.py`, guarding against empty string values.

39. **Self-Deadlock on Configuration Save Error in `main.py`** — *RESOLVED*:
    - Moved `save_backup_config(payload)` outside of `with DEPLOY_LOCK:`. If an exception occurs when saving YAML, `log_event()` acquiring `LOG_CONDITION` (which uses `DEPLOY_LOCK`) no longer deadlocks the HTTP server thread.

40. **Unchecked Inbound API Addition in `panel/setup.sh`** — *RESOLVED*:
    - In `panel/setup.sh: add_inbound()`, added verification of `"success":true` in `./working/add-inbound-response.json` with fail-fast `die` on errors, mirroring `add_client()`.

41. **Unquoted Non-ASCII Client Name in Sub-Server Fetch URL** — *RESOLVED*:
    - In `sub-server/server.py`, wrapped `client` with `urllib.parse.quote(client, safe='')` when fetching subscriptions from backend nodes.

42. **Special Characters in Domain Corrupting `sed` Replacement during Recovery** — *RESOLVED*:
    - In `ssh_deployer.py: recovery`, safely escaped sed delimiter and metacharacters (`\`, `/`, `&`) in `$OLD_DOMAIN` and `$NEW_DOM`.

43. **Brute-force Protection for Sub-Server Admin Login** — *RESOLVED*:
    - Added in-memory per-IP rate-limiting and temporary lockout window for failed login attempts in `sub-server/server.py`.

44. **Input Validation and Robustness Hardening in Sub-Server and Frontend** — *RESOLVED*:
    - Handled non-dict JSON body payloads in `sub-server/server.py` (`isinstance(data, dict)`).
...
## Nice to Have / Architectural Refactoring & Code Cleanliness

The following items are strategic architectural improvements and refactoring opportunities to eliminate technical debt, reduce file sizes, and enforce separation of concerns across the project:

1. **Frontend Modularization (`panel/static/app.js`)**:
   - *Problem*: `app.js` is a single ~3800-line monolith combining WebCrypto (PBKDF2/AES-GCM), modal management, server profile drawer, form validation for 12 modes, dynamic SVG topology rendering, and SSE log streaming.
   - *Target Architecture*: Split into native ES modules:
     - `modules/crypto.js` — Encryption, master password key derivation, auto-migration.
     - `modules/servers.js` — Saved VPS profile drawer, locking, editing, quick-select.
     - `modules/logs.js` — Server-Sent Events (SSE) stream, auto-scroll, status badges.
     - `modules/forms.js` — Mode toggles, validation rules, state persistence.
     - `modules/topology.js` — SVG architecture diagram renderer.

2. **Template Extraction from Subscription Server (`sub-server/server.py`)**:
   - *Problem*: `server.py` contains ~2000 lines of inline HTML, CSS, SVG icons, and JS embedded as raw multiline Python strings with manual `.replace("__VAR__", ...)` substitutions.
   - *Target Architecture*: Move static markup and styles to `sub-server/templates/web/` (`dashboard.html`, `login.html`, `style.css`), reducing `server.py` from ~3400 lines to ~350 lines of clean routing and proxy logic.

3. **Externalization of Embedded Shell Scripts in `ssh_deployer.py`**:
   - *Problem*: Multi-hundred-line Bash scripts (`PANEL_DOMAIN_REWRITE_SCRIPT`, `LEGACY_COMPOSE_REWRITE_CMD`, etc.) are written as multiline Python strings inside `ssh_deployer.py`, preventing shell syntax highlighting, linting (`shellcheck`), and automated testing.
   - *Target Architecture*: Place remote scripts into standalone `.sh` files under `common/remote_scripts/` or `panel/scripts/` and load/stream them to remote targets.

4. **Target Server Data Model Unification**:
   - *Problem*: Form state and deployers use 7+ distinct sets of prefixed keys (`vps_*`, `freedom_*`, `proxy_*`, `sub_vps_*`, `backup_vps_*`, `recovery_vps_*`, `update_vps_*`) representing identical SSH connection parameters, leading to redundant field mapping and validation code.
   - *Target Architecture*: Adopt a unified `ServerConnection` data model (`host`, `port`, `user`, `auth_type`, `password`, `key`) with reusable frontend server selector components.

5. **Splitting Orchestration Logic (`ssh_deployer.py`)**:
   - *Problem*: `ssh_deployer.py` is ~1550 lines combining SSH execution, archive bundling, panel deployment, cascade orchestration, sub-server management, backup generation, and container recovery.
   - *Target Architecture*: Decompose into:
     - `core/ssh_client.py` — Low-level async SSH/SCP wrapper.
     - `deployers/panel_deployer.py` — Single & cascade node setup.
     - `deployers/sub_deployer.py` — Subscription server lifecycle.
     - `deployers/maintenance.py` — Backup, recovery, restart, and update routines.

6. **Strategy Pattern for Deployment Modes (Open/Closed Principle)**:
   - *Problem*: Adding or modifying a deployment mode requires editing cascading `if/elif/else` blocks across 5+ files (`app.js`, `main.py`, `ssh_deployer.py`, `index.html`), violating OCP.
   - *Target Architecture*: Implement a pluggable `DeploymentStrategy` registry where each mode encapsulates its own parameter schema, validation rules, multi-stage execution pipeline, and result formatter.

7. **Strongly Typed Domain Models & Validation (Type Safety & Fail-Fast)**:
   - *Problem*: Configuration dictionaries are passed around as untyped `Dict[str, Any]` and loose JS objects. Key typos (e.g. `update_vps_host` vs `update_host`) silently resolve to `None`/`""` without runtime warnings.
   - *Target Architecture*: Adopt typed data models (`dataclasses` / `pydantic` in Python and typed schemas in JS) to validate contracts at the API and orchestration boundaries.

8. **Testability via Transport Abstraction (Dependency Inversion Principle)**:
   - *Problem*: Deployers directly shell out to system `ssh`/`scp` subprocesses, making automated unit testing impossible without live VPS instances (the `tests/` directory remains empty).
   - *Target Architecture*: Introduce an abstract `RemoteTransport` interface (`SSHTransport`, `MockTransport`, `DryRunTransport`) enabling fast unit tests and pipeline dry-runs.

9. **Structured Event Logging & Observability**:
   - *Problem*: Logs use unstructured text strings with ad-hoc prefixes (`[ERROR]`, `[OK]`, `[CANCEL]`) and bracketed markers (`===RESULT_JSON_START===`), parsed via string searching and regex.
   - *Target Architecture*: Transition SSE log streaming to structured event objects (`{ timestamp, level, stage, message, payload }`) with type-safe rendering in the frontend.

10. **Declarative State Reconciliation & Idempotency (IaC Principles)**:
    - *Problem*: `panel/setup.sh` is imperative and destructive (`rm -rf ./working`, regenerating all Reality keys and port allocations on every run).
    - *Target Architecture*: Inspect current container and configuration state before modifying files or generating secrets, allowing safe, non-destructive re-executions of setup scripts.
