...
> Статус: в процессе. **Фазы A и B + C1 + C3 + C4 + C5 + D1 + E1 + F1 выполнены** (A — защитный слой unit-тестов и фиксация контрактов; B — шаблонизация `sub-server/server.py`; C1 — maintenance-срез `ssh_deployer.py` → `deployers/maintenance.py`; C3 — deploy-ветки → `deployers/panel_deployer.py` + `deployers/sub_deployer.py`; C4 — сигнатуры `_deploy_node`/`_deploy_sub_server` → `NodeConfig`; C5 — `SSHDeployer` → `core/ssh_client.py`; D1 — маршруты `main.py` → `routing.py`; E1 — все модули `app.js` → `panel/static/modules/`; F1 — `ServerConnection` dataclass + `pick_ssh` в `save_backup_config`).
...
### C2. Вынести embedded bash/awk в файлы — [ВЫПОЛНЕНО]
- `PANEL_DOMAIN_REWRITE_SCRIPT` → `common/remote_scripts/panel_domain_rewrite.sh` (6781 байт).
- `LEGACY_COMPOSE_REWRITE_CMD` → `common/remote_scripts/legacy_compose_rewrite.sh` (590 байт).
- `LEGACY_TEMPLATES_SYMLINK_CMD` → `common/remote_scripts/legacy_templates_symlink.sh` (197 байт).
- Извлечено программно (ast.literal_eval) — байт-точное совпадение, сверено после загрузки.
- `ssh_deployer.py`: константы теперь читаются через `_read_remote_script()` (`os.path.join(REPO_ROOT, "common", "remote_scripts", ...)`), имена сохранены — `deployers/maintenance.py` импорты не менялись.
- `get_bundle_bytes()`: подтверждено — все 3 файла попадают в tar-бандл (exclusion list не трогает `common/`).
- Валидация: `bash -n` для всех 3; py_compile; значения загруженных констант === содержимое файлов (MATCH ×3); unit-тесты (10+17) зелёные.
...
### C6. Константы наружу (F10) [ВЫПОЛНЕНО (Python-сторона)]
- Создан `constants.py`: `REMOTE_DIR = "/opt/3x-ui-bootstRUp"`, `PANEL_CONTAINER = "3xui"`, `PANEL_API_PORT = "2053"`.
- `ssh_deployer.py` (`_deploy_node`, `_deploy_sub_server`) и `deployers/maintenance.py`: plain-Python `remote_dir = REMOTE_DIR` (5 вхождений), `_sub_server_sync_cmd(REMOTE_DIR, ...)` (1).
- Embedded bash-строки (`cd /opt/...`, `WORK_DIR="..."`, `caddy_cmd`) намеренно НЕ тронуты — remote IaC.
- Без изменений поведений: `py_compile` OK, импорты OK, 0 plain-Python вхождений осталось.
- `common/constants.sh` (bash-зеркало) — отложено до решения по remote IaC (см. «Что НЕ делать»); в setup.sh значения оставлены.
...
- [ВЫПОЛНЕНО] `modules/ui.js` создан (ui + md5 + derive, verbatim-копия строк 1–303 app.js, подключён обычным `<script>` перед app.js — глобальные функции сохранены); app.js 5460 → 5157 строк. `modules/crypto.js` создан (deriveKeyV1/V2, vault cookie, getPayloadVersion, encryptData/decryptData — 8 чистых crypto-функций, window-экспорт для app.js); app.js 5157 → 5081 строк. `modules/topology.js` создан (renderTopologyDiagram, 510 строк verbatim + window-экспорт); app.js 5081 → 4573 строки. `updateSecretHashPreviews` вынесена в ui.js (top-level, глобальная через function declaration); app.js 4573 → 4528. `copyToClipboard` вынесена в ui.js (function declaration, глобальная); app.js 4528 → 4515. `modules/logs.js` создан (appendLog + initProgressIndicator + updateProgressIndicator, 105 строк verbatim + window-экспорт + терминал-листенер); app.js 4515 → 4410. `fetchBackupList` вынесена в ui.js (async function); app.js 4410 → 4346. QR-модалка (showQrModal/hideQrModal + DOM-const + listeners, 64 строки verbatim + window-экспорт) вынесена в ui.js; app.js 4346 → 4276. Changelog+Happ Routing модалки (179 строк verbatim + window-экспорт) вынесены в ui.js; app.js 4276 → 4097. `modules/servers.js` создан (весь Server Manager Sidebar Logic: sm*-const, драг-сортировка, cryptoKey/serversList state, fetchServers/saveServersToBackend/renderSavedServers/promptMasterPassword/submitMasterPassword/restoreVaultKeyFromCookie/setEditingMode/doSaveServer/initServerDropZones + модалки, 1013 строк verbatim + window-экспорт 11 функций/аксессоров ВНУТРИ DOMContentLoaded); app.js 4097 → 3091. Внешние ссылки переведены на мосты: window.getServersList()/setServersList() (авто-save panel_url при деплое), window.saveServersToBackend/renderSavedServers, window.initServerDropZones/fetchServers, window.__updateBackupName, window.__getGlowRefresher. SSE-стрим деплоя (`startDeployLogStream(onDone)`, 11 строк verbatim + window-экспорт, EventSource+onmessage/onerror) и `btnCopyLogs` listener (7 строк verbatim) вынесены в logs.js; app.js 3091 → 3066. Forms-блок (getSelectedMode, updateStep3Header, updateModeUI + радио-listener, wizard showStep + кнопки шагов, glow-валидация REQUIRED_FIELDS/collectPasswordErrors/applyGroupGlow/getErrorFieldIds/applyGlowForEmptyFields/validateRequiredPasswords/getFieldValue/applyPasswordGlow/clearPasswordGlow) вынесен в forms.js (~646 строк, все verbatim), app.js 3066 → 2409; мосты `window.__resetSSHValidation`, `window.__getGlowRefresher` + экспорты `window.getSelectedMode/updateModeUI/showStep/getFieldValue/applyGlowForEmptyFields/validateRequiredPasswords`; index.html подключён forms.js после servers.js. `modules/deploy.js` создан (весь deploy-блок: btnStartDeploy/btnStopDeploy константы+listeners, сборка payload для всех 12+ режимов, checkFinalStatus c рендером карточек/клиентов/авто-save panel_url, 792 строки verbatim + window.checkFinalStatus); app.js 2409 → 1640 строк; мосты window.__getLatestFetchedXuiVersion/__saveCurrentConfig; terminalLogs взят из глобальной области logs.js (дубль-объявление удалено), мёртвая isUserScrolledUp удалена; index.html подключён deploy.js после forms.js перед app.js. E1: все модули вынесены.
...
### E2. Убрать глобальную подмену `window.alert`/`confirm` — [ВЫПОЛНЕНО]
- Удалены `window.alert = ...` и `window.confirm = ...` из `modules/ui.js` (12 строк).
  `showAlert`/`showConfirm` остаются глобальными (window-экспорт) — прямая замена.
- Единственный вызов `alert(...)` в `forms.js` переведён на `showAlert(...)`; bare `alert()`/`confirm()`
  в `panel/static` больше нет (проверено grep).
- Нативные `window.alert`/`window.confirm` восстановлены как есть браузерные — код панели их не использует.
- Проверено: `node --check` чист, Playwright E2E `test_ui_button_notifications` зелёный
  (кастомные модалки + 0 native dialogs), 0 pageerrors.

---

## Фаза F: единая модель сервера (сквозная проблема)

### F1. Unified `ServerConnection` — [ВЫПОЛНЕНО]
- Сейчас 7 наборов ключей (`vps_*`, `freedom_*`, `proxy_*`, `sub_vps_*`, `backup_vps_*`, `recovery_vps_*`, `update_vps_*`).
- **Действие**: единая dataclass `ServerConnection` (host, port, user, password, key) + маппинг старых ключей → модель на границе парсинга формы.
- **Сделано**:
  - `@dataclass ServerConnection` в `ssh_deployer.py` (рядом с `NodeConfig`): `from_config(config, prefix)` читает `<prefix>_host/_port/_user/_password/_key`; `validate(label)` возвращает ровно те же сообщения, что legacy `_check_ssh`.
  - `validate_deployment_config`: `_check_ssh(host_key, port_key, user_key, pass_key, key_key, label)` c 6 аргументами → `_check_ssh(prefix, label)`; 10 вызовов сокращены (`vps`, `freedom`, `proxy`, `sub_vps` ×4, `backup_vps`, `recovery_vps`, `update_vps`). Вся SSH-логика валидации теперь в одном месте.
  - `main.py: save_backup_config`: добавлен `pick_ssh(prefix, auth_type_key, *extra)`; 7 секций (`freedom_node`, `proxy_node`, `standard_node`, `sub_server`, `backup_node`, `recovery_node`, `update_node`) переведены с ручных списков на него. `panel_and_clients` не тронут (не SSH-роль).
  - **Нюанс**: `*_auth_type` НЕ единообразен — `sub_auth_type`, `backup_auth_type`, `recovery_auth_type`, `update_auth_type` без `_vps` (в отличие от `vps_auth_type`/`freedom_auth_type`/`proxy_auth_type`). Поэтому `pick_ssh` принимает auth_type_key явно; унификация имён решена в F2 (authBase = `prefix.endsWith('_vps') ? prefix.slice(0, -4) : prefix`).
  - **Проверка**: эквивалентность `save_backup_config` до/после (те же 80 не-секретных ключей в тех же секциях, секреты не пишутся) + 25 unit (validation + core signatures) зелёные. Формат `setup_backup.yml` не изменён (обратная совместимость).
...
### F2. Переиспользуемый frontend-селектор сервера [ВЫПОЛНЕНО]
- Вынести повторяющиеся поля ввода SSH в один HTML-компонент/шаблон, вместо копипасты по секциям формы.
- Решение: `<template id="tpl-ssh-fields">` с плейсхолдерами `{{P}}` (prefix) / `{{A}}` (authBase) + JS-рендер `renderSshFields()` в `forms.js` DOMContentLoaded.
- 7 секций (`data-ssh-prefix` хуки): vps, freedom, proxy, sub_vps, backup_vps, recovery_vps, update_vps.
- authBase derivation: `prefix.endsWith('_vps') ? prefix.slice(0, -4) : prefix` — покрывает non-uniform id-имена (`subPassGroup` vs `vpsPassGroup`).
- Все 56 IDs + 7 auth-toggle работоспособны, 0 pageerrors, `node --check` чист.

---

## Фаза G: стратегический паттерн режимов (опция, самое дальнее)

### G1. Python-реестр стратегий — [ВЫПОЛНЕНО]
- Создан `deployers/strategies.py`: `ModeStrategy` (modes, validate, run) + `RunContext`
  (bundle_dir, change_ssh_port, new_ssh_port, updated_ssh_ports, prepare_decoy_files).
- 9 стратегий → плоский `STRATEGIES: Dict[str, ModeStrategy]` (18 модов, все покрыты).
  Группы с общим кодом делят валидатор/runner (`single/freedom_component` → `deploy_single`;
  `update/update_3xui` → `deploy_update`; `update_vps`-группа и sub-ops — отдельные runners).
- `ssh_deployer.py`: `validate_deployment_config` — lookup `STRATEGIES[mode].validate(config)`;
  `run_deployment` — lookup `.run(config, log, cancel_check, ctx)`. Дублирующие if/elif удалены
  (dispatch-блок 958-1038: 81 строка → 14).
- Импорт ленивый (`from ssh_deployer import ServerConnection` внутри `_check_ssh`) — циклов нет.
- **Поведенческая сверка**: 28 конфигов старой (git HEAD) vs новой валидации — 27/28 идентичны.
  Расхождение на `bogus_mode`: старый код пропускал неизвестный режим (валидация True → dispatch
  падал в конец → `None`), новый — явная ошибка "Неизвестный режим деплоя". Осознанный багфикс
  (тихий fail → явный), тесты на него не завязаны.
- Dispatch-тест: 18 модов → правильный деплойер + правильный kwarg-набор (mock-сверка).
- `py_compile` + 40 unit-тестов зелёные. Поведение деплоя не менялось.

### G2. Фронтенд-валидация через `/api/modes` — [ВЫПОЛНЕНО]
- Каскадные `if/elif` по `mode` в `app.js`/`forms.js` (REQUIRED_FIELDS) — нарушение Open/Closed.
- **Действие**: endpoint `/api/modes` отдаёт schema валидации из `STRATEGIES` (поля, обязательность,
  группы any-полей); `forms.js` строит REQUIRED_FIELDS динамически вместо хардкода.
- Риск: высокий (фронтенд — 12 режимов, E2E-покрытие). Зависит от G1 (готово).
- Реализация: `MODES_SCHEMA` (52 поля + 2 группы, `kind: group|field`, SSH password/key поля
  помечены `authField`/`authValue`) в `deployers/strategies.py`; `routing.py` — `GET /api/modes`
  → `{"modes": MODES_SCHEMA}`; `forms.js` — `loadRequiredFields()` (memoized fetch, lazy) собирает
  REQUIRED_FIELDS, `when` для auth-полей регенерируется из authField/authValue;
  `validateRequiredPasswords` стал `async` (await схемы до первой валидации), `deploy.js` — `await`.
- **Поведенческая сверка**: 52 поля + 2 группы из схемы = старый REQUIRED_FIELDS из git HEAD
  (ids, порядок, step, modes, message идентичны; when-поля ≡ auth-пары по исходному JS-тексту).
- `/api/modes` smoke-проверен на живом сервере (54 правила). `node --check` + `py_compile` чисты,
  40 unit-тестов зелёные. Поведение валидации не менялось.
...
| 3 | C1/C3 (orchestration-срезы) | высокая | средний | **C1+C3 ВЫПОЛНЕНО** (maintenance → `deployers/maintenance.py`; panel/cascade → `deployers/panel_deployer.py`; sub-only → `deployers/sub_deployer.py`). **C6 ВЫПОЛНЕНО** (Python-сторона: `constants.py` + `REMOTE_DIR` в 6 точках; embedded bash не тронут). **C2 ВЫПОЛНЕНО** (3 bash → `common/remote_scripts/*.sh`, байт-точное извлечение, `bash -n` OK; `ssh_deployer.py` → 1038 строк). |
| 4 | C5 (SSH-транспорт) | высокая | средний | **ВЫПОЛНЕНО** (`SSHDeployer` → `core/ssh_client.py`) |
...
| 6 | F (модель сервера) | средняя | средний | **F1+F2 ВЫПОЛНЕНО** (`ServerConnection` + `_check_ssh(prefix, label)` + `pick_ssh`; `<template>` + `renderSshFields()` — 7 секций, 56 IDs) |
| 7 | E (app.js) | высокая | высокий | **E1+E2 ВЫПОЛНЕНО** (8 ES-модулей: ui/crypto/topology/logs/servers/forms/deploy; app.js 5460→1638; E2: window.alert/confirm совместимость убрана) |
| 8 | G (стратегии) | опция | высокий | **G1+G2 ВЫПОЛНЕНО** (9 `ModeStrategy` → `STRATEGIES` на 18 модов в `deployers/strategies.py`; `validate_deployment_config`/`run_deployment` — data-driven; 28-кейс сверка, 1 осознанный багфикс bogus_mode; 18-мод mock-dispatch + 40 тестов зелёные). **G2**: `GET /api/modes` + `MODES_SCHEMA` (52 поля + 2 группы) → `forms.js` строит REQUIRED_FIELDS динамически; 52+2 сверены со старым массивом 1:1 |
