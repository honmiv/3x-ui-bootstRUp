...
async def deploy_recovery(
    ...
    target_domain = (config.get("recovery_domain") or config.get("domain") or host).strip()
    domain_changed = bool(old_domain) and old_domain.lower() != target_domain.lower()
    if domain_changed and not recovery_creds_provided:
        log(f"[ERROR] Domain change detected in the backup (old: '{old_domain}' -> new: '{target_domain}'). "
    ...
    async with SSHDeployer(host, port, user, password, key_data, cancel_check=cancel_check) as deployer:
        ...
        new_domain_str = shlex.quote(target_domain)
        remote_script = (
            "set -e\n"
        ...
        if domain_changed:
            log(f"Rewriting panel domain '{old_domain}' -> '{target_domain}' via the 3x-UI HTTP API...", "info")
            ...
            rewrite_env = (
                ...
                f"RECOVERY_NEW_DOM={shlex.quote(target_domain)} "
        ...
        xui_url = f"https://{target_domain}/{web_base_path}/" if web_base_path else f"https://{target_domain}/"
...
async def deploy_sub_ops(
    ...
    async with SSHDeployer(sub_host, sub_port, sub_user, sub_password, sub_key, cancel_check=cancel_check) as deployer:
        ...
        if deploy_mode == "restart_sub":
            ...
            sync_cmd = _sub_server_sync_cmd(REMOTE_DIR, preserve=True)
            rc, sync_out = await deployer.exec_command(sync_cmd, lambda m: log(m, "info"), stdin_data=bundle_bytes)
            if rc != 0:
